			...
			if (isset($item['type'])) {
				switch ($item['type']) {
					...
					case 'card-configTable':
						...
						break;
					case 'tableDataInput':
						$data = core::$model->Config->read($item['configName'], $item['default'] ?? '');
						$this->addData('<tr>
                            <td>' . ($item['name'] ?? $item['configName']) . '</td>
					...
					case 'tableDataCheckBootstrap':
						...
						$this->addData('<tr>
                            ...
                            <td class="custom-control custom-switch">
                                <input type="checkbox" class="custom-control-input" id="' . $idElement . '" ' . ($data ? 'checked' : '') . '>
